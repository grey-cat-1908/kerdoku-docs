...
- В раздел экономики добавлена новая игра `rps` (Камень-Ножницы-Бумага). Она поддерживает как оффлайн-режим (похоже на `moneybag`), так и режим игры с другим игроком.
- Исправлен баг с игрой `jackpot`, позволяющий украсть деньги в момент игры, а тем самым преумножить их.
- Созданы игровые сессии, что поможет не потерять баланс во время технических работ или начала других игр.

**__Bonus__**
- Улучшена система розыгрышей: Дизайн, Обработка победителей.

**__Прочее__**
- Исправлена ошибка в команде `unmute`
- Исправлена ошибка в команде `profile`
- Прочие исправления.

### HotFix 4.2.1
**__Веб-сайт__**

- В панели управления сервером добавлена кнопка "Мета-Данные". (в случае каких-либо проблем с панелью пользователи могут с легкостью передать подробную информацию о сессии панели).
- Исправлена ошибка с настройкой команды `rob`.
- Некоторые технические изменения и исправления.
- Поддержка измененных передаваемых ботом данных.

**__Бот__**

- Бот теперь передает сайту некоторые новые важные данные.
- В команде `botinfo` исправлена дата.
- Переработана команда `serverinfo`
- Исправлены строчный ввод в командах `deposit` и `withdraw`.
- К команде `balance` добавлен алиас `bal`
...
- Добавлена настройка фильтра известных фишинг-ссылок:
    ...
    - Каналы-Исключения для работы фильтра.
